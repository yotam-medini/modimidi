...
class AbsEvent {
 public:
  ...
  virtual void SetSendFluidEvent(
    fluid_event_t *event, const Player::Impl *player, uint32_t date_ms) = 0;
...
class NoteEvent : public AbsEvent {
 public:
  ...
  void SetSendFluidEvent(
    fluid_event_t *event, const Player::Impl *player, uint32_t date_ms);
...
class CallBackData {
 public:
  enum CallBack { Periodic, Final, Interactive };
  CallBackData(CallBack ecb, Player::Impl *player) :
    ecb_{ecb}, player_{player} {}
  CallBack ecb_;
  Player::Impl *player_;
};
...
class Player::Impl {
 public:
  enum SeqId : size_t {
    SeqIdSynth, SeqIdPeriodic, SeqIdFinal, SeqIdInterActive, SeqId_N };
  Impl(
      const midi::Midi &pm,
      SynthSequencer &ss,
 ...
 private:
  ...
  void ScheduleInterActiveAt(uint32_t at) {
    ScheduleCallback(seq_ids_[SeqIdInterActive], at);
  }
  void ScheduleCallback(int seq_id, uint32_t at);
  void ProgressHandle(unsigned int time);
  void RemoveEvents();
...
void Player::Impl::SetVelocitiesMap() {
...
uint32_t Player::Impl::GetFirstNoteTime() {
  uint32_t t = 0;
...
uint32_t Player::Impl::GetNoteDuration(
    size_t iei,
...
uint8_t Player::Impl::MapVelocity(
    const midi::NoteOnEvent *note_on,
...
void Player::Impl::ScheduleCallback(int seq_id, uint32_t at) {
...
void Player::Impl::Callback(
    unsigned int time,
    fluid_event_t *event,
    fluid_sequencer_t *seq,
    CallBackData::CallBack ecb) {
  if (in_pause_) {
    InteractiveCallback(time, event, seq);
  } else {
    switch (ecb) {
     ...
     case CallBackData::CallBack::Final:
      ...
      break;
     case CallBackData::CallBack::Interactive:
      InteractiveCallback(time, event, seq);
      break;
     default:
...
void Player::Impl::PeriodicCallback(
    unsigned int time,
    fluid_event_t *event,
    fluid_sequencer_t *seq) {
...
void Player::Impl::FinalCallback(
    unsigned int time,
    fluid_event_t *event,
    fluid_sequencer_t *seq) {
...
void Player::Impl::InteractiveCallback(
    unsigned int time,
    fluid_event_t *event,
    fluid_sequencer_t *seq) {
  // if no action.....
  ...
  ScheduleInterActiveAt(time_next);
}
...
void NoteEvent::SetSendFluidEvent(
  ...
  fluid_event_set_dest(event, player->GetSeqId(Player::Impl::SeqIdSynth));
...
void FinalEvent::SetSendFluidEvent(
    fluid_event_t *event, const Player::Impl *player, uint32_t date_ms) {
  fluid_event_set_source(event, -1);
  fluid_event_set_dest(event, player->GetSeqId(Player::Impl::SeqIdFinal));
  fluid_sequencer_send_at(
    player->GetSynthSequencer().sequencer_, event, date_ms, 1);
}
